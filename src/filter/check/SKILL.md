...
Goal: Determine pass/partial/fail status of an object, result or outcome against defined target or acceptance criteria or requirements.
Non-Goals: Fixing failures, implementing solutions, or making final go/no-go decisions.
Use-When: Use when you need to validate whether an object, solution, result or outcome meets its target or acceptance criteria.

## 0. Prerequisites
- Prompt, context or files with the object, solution, result or outcome to evaluate (e.g. prototype, PoC, test results)
- Prompt, context or files with the requirements, acceptance criteria or target outcomes to evaluate against (e.g. Specifications, Plans, Documentation, Hypotheses)

## 1. Inputs
- Object, solution, result or outcome to evaluate (e.g. prototype, PoC, test results)
- Requirements, acceptance criteria or target outcomes to evaluate against (e.g. from `define`, `synthesize`, or project documentation)
...
5. Capture any partial successes or failures, and document confidence (high, medium, low) and impact (high, medium, low) for each requirement
6. Capture if there are blockers or dependencies which indicate an error in the requirements, acceptance criteria or target outcomes themselves
...
**Outcome:** Prompt output with hypothesis evaluation: supported/not supported/insufficient evidence, plus high/medium/low confidence level and data needed for further checks.
